...
**Source:** https://sumproduct.com/thought/allocating-fees/
...
I shall be seeking to model forecast fees for the calendar year.

Allocating Fees
===============

I shall be seeking to model forecast fees for the calendar year 2021. There are five terms relevant to your modelling period:

![](https://sumproduct.com/wp-content/uploads/2025/05/e774d10cbbb9450fc45efbe51abdf434-602.jpg)

In [Coming to Terms with Dates](https://www.sumproduct.com/thought/coming-to-terms-with-dates)
, we allocated the number of term days (including weekends and public holidays) to each month of 2021, _i.e._

![](https://sumproduct.com/wp-content/uploads/2025/05/f32e5a15e2cf9c3e4d2d058458ce054d-621.jpg)

This time, I need to extend the idea to construct a calculation such that the correct fees will be allocated to each month. Similar to last time, I shall assume the terms will be in chronological order, that they will not overlap, there will never be more than two terms associated with any given month, and there will never be two start dates or two end dates in the same month.

To reassure you, there are no plans to recreate the wheel here. The solution from [Coming to Terms with Dates](https://www.sumproduct.com/thought/coming-to-terms-with-dates)
 will be exploited fully, as essentially all we have to do is add in a few more calculations. And that begins immediately with the inputs: did you notice that the total of the fees for the 12 months in our screenshot _(above)_ ($29,020) did not equal the total fees for the four terms ($39,000) in the previous image?
...
![](https://sumproduct.com/wp-content/uploads/2025/05/f1140ff857fc3b6f5f97a6a24f4a6fc7-337.jpg)
...
From [Coming to Terms with Dates](https://www.sumproduct.com/thought/coming-to-terms-with-dates)
 (which we have modified here), the idea is simple. I just need to calculate the number of days of each term are in each month, and then multiply the number of days identified by the daily rate _(already calculated, above)_. There is just one complication: which term?

This is where we need to go back to the reasoning I used in [Coming to Terms with Dates](https://www.sumproduct.com/thought/coming-to-terms-with-dates)
. There are three scenarios for each term:

*   No term days are in the reporting period

![](<Base64-Image-Removed>)

*   At least one of the start and / or end dates for a given term is in the reporting period (start date must be less than or equal to the end date)
...
*   One term ends and the next term starts within the reporting period (start date occurs after the end date for that reporting period).
...
With all this borne in mind, we are now in a position to model the problem, and our solution is attached [here](https://sumproduct.com/assets/thought-files/allocating-fees/sp-allocating-fees.xlsm)
.
...
The **CHOOSE** function was covered previously. Essentially, there are three scenarios:

*   If no term dates occur (**J$47** is equal to 1), then the value is zero (no days)
*   If one term’s dates are (partially) included in the month (**J$47** is equal to 2), then the number of days is given by **MIN(J$7,J$46)-MAX(J$6,J$45)+1**

![](<Base64-Image-Removed>)

_i.e._ the number of days is the lower of the month end date (**y**) and the term end date (**t2**) less the higher of the month start date (**x**) and the term start date (**t1**) plus one (so the first day of the term dates is added back)

*   If two terms’ days are included (**J$47** is equal to 3), then the number of days is given by **J$46-J$6+1**

![](<Base64-Image-Removed>)

_i.e._ the number of days is given by the first term end date (**t2**) less the month start date (**x**) plus one (to again add the first day of the term days back).
...
I am conscious there are other ways to calculate the solution, but I wanted to build on the approach in [Coming to Term with Dates](https://www.sumproduct.com/thought/coming-to-terms-with-dates)
. It’s important to re-use calculations where possible, and not reinvent the wheel pointlessly.

[More Thoughts](https://www.sumproduct.com/thought)

*   [Log in](https://sumproduct.com/thought/allocating-fees/#0)
    
*   [Register](https://sumproduct.com/thought/allocating-fees/#0)
...
[Forgot your password?](https://sumproduct.com/thought/allocating-fees/#0)
...
[Back to login](https://sumproduct.com/thought/allocating-fees/#0)

[](https://sumproduct.com/thought/allocating-fees/#0 "close")
