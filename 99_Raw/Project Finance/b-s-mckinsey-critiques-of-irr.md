# IRR, IRR Alternatives and McKinsey B.S.

**Source:** https://edbodmer.com/b-s-mckinsey-critiques-of-irr

---

In this webpage I demonstrate real problems with the IRR and contrast this to a B.S. article published a few years ago by McKinsey that strongly critiqued the IRR. The best way to fix the IRR is to reconcile the IRR with the return on investment and then use your own cost of capital in a weighted average ROI  calculation. This weighted average ROI calculation solves all sorts of problems with the IRR.  I demonstrate that the weighted ROI is much better than the MIRR.  Indeed the MIRR just distorts things and improves the re-investment rate problem of the IRR.

I show that a real problem with the IRR is changing discount rates driven by changing which are not generally addressed in NPV analysis either. These issues of changing discount rates were certainly not discussed by McKinsey. The problem of changing risk over time can be resolved by assuming different sale dates with assumed required IRR’s.  Another really big problem with the IRR is undervaluation of long-term cash flows when the IRR is high.  This problem is solved with use of the weighted average ROI.

Critiques of IRR and McKinsey B.S. Article
------------------------------------------

I have recently listened to complaints about the IRR statistic from some of participants in my courses.  Students apparently learn these problems in business school.  My friends have also sent me academic articles about problems with the IRR from McKinesy.  These young people who point to the McKinsey article imply that the IRR is bad because:
...
Somebody who copied my MIRR function even stated:”Despite the arguable superiority of the **Modified Investment Rate of Return (MIRR)** in analyzing investments or capital budgeting decisions …” I will show that this idea that the MIRR is superior to IRR is complete and utter B.S. ([Note that you can find documentation of how to create a UDF function on this website.](https://edbodmer.com/xmirr-udf-function/)
)

Instead of using a statistic that nobody uses (i.e. the MIRR), I demonstrate on this web page the following:

*   The IRR can be reconciled with the ROI by computing an investment base and then discounting this investment base at the cost of capital.   The discounted investment base is then used to weight the ROI in an weighted average calculation. If the discount rate is the IRR itself, the IRR and the weighted average ROI produce the same result.  If the discount rate is the cost of capital and not the IRR itself, the product of the weighted average ROI and the nominal invested capital produce the NPV of cash flows.
*   The IRR can indeed give you different answers relative to the NPV.  The only reason this occurs is that the middle period cash flows are given more value in the IRR calculation than the NPV calculation.  The MIRR solves this problem when time periods are the same but it fails when time periods are different.
*   The MIRR does not give you a better indication of rate of return on an investment because the MIRR just pushes numbers towards the assumed re-investment rate. As higher risk investments should put lower weight on out year cash flows, the higher re-investment rate seems to imply an investment has a higher value when just the opposite should occur.
*   When the weighted ROI is used as a statistic rather than the IRR or the MIRR, the ranking of investments is the same as with the NPV and all of the really big problems with the MIRR are avoided.
*   The weighted average ROI statistic solves the problem that the IRR undervalues long-term cash flows in high IRR investments. When a high IRR is used to measure long-term investments, the implied high discount rate gives very little weight to out-year cash flow.  But when the weighted average ROIC is computed, this problem does not occur.

To introduce issues associated with the IRR, I start with a simple example and explain the mechanics of the IRR, the MIRR, the compound growth rate, the NPV and the weighted average ROI.  One of the ideas of this page is that you can explain the different statistics with very simple focused examples.  After explaining the statistics with the simple examples I use some concentrated examples to illustrate the investment ranking issue, the re-investment rate, the multiple returns.

You can find the examples of how the weighted average ROI solves problems in both the IRR and the MIRR using the file below.  This file includes the examples used in this webpage to illustrate various valuation issues.
...
Demonstration of IRR, MIRR, NPV and Weighted ROI using Simple Three Period Example
----------------------------------------------------------------------------------
...
Now lets move to the case with middle year cash flows.  I have constructed a very extreme example to demonstrate problems with the re-investment rate. In this example I assume that the second period cash flow is 160 and the third year cash flow is 70. These cash flows produce a very high IRR of 95.76%. If a 10% discount rate is applied, the MIRR is much less.  The MIRR is 56.84% as demonstrated in the screenshot below.  (You can watch to video below to see how these statistics are computed and how to colour the sheet).
...
[https://www.mckinsey.com/business-functions/strategy-and-corporate-finance/our-insights/internal-rate-of-return-a-cautionary-tale](https://www.mckinsey.com/business-functions/strategy-and-corporate-finance/our-insights/internal-rate-of-return-a-cautionary-tale)
...
![](https://pixel.wp.com/g.gif?v=ext&blog=182904628&post=4971&tz=0&srv=edbodmer.com&j=1%3A13.2.3&host=edbodmer.com&ref=&fcp=8648&rand=0.690536799110593)