...
**Source:** https://exceljet.net/formulas/vlookup-with-numbers-and-text

---

[Skip to main content](https://exceljet.net/formulas/vlookup-with-numbers-and-text#main-content)

[](https://exceljet.net/formulas/vlookup-with-numbers-and-text#)

*   [Previous](https://exceljet.net/formulas/vlookup-with-multiple-criteria-advanced)
    
*   [Next](https://exceljet.net/formulas/vlookup-with-two-client-rates)
...
by Dave Bruns · Updated 9 May 2022
...
![Excel formula: VLOOKUP with numbers and text](https://exceljet.net/sites/default/files/styles/original_with_watermark/public/images/formulas/VLOOKUP%20with%20numbers%20and%20text.png "Excel formula: VLOOKUP with numbers and text")

Summary
-------

To use the [VLOOKUP function](https://exceljet.net/functions/vlookup-function)
 to look up information in a table where the first column contains numbers that are actually text, you can use a formula that [concatenates](https://exceljet.net/glossary/concatenation)
 an [empty string](https://exceljet.net/glossary/empty-string)
 ("") to the numeric lookup value, coercing it to text. In the example shown, the formula in H3 is:

    =VLOOKUP(id&"",planets,2,0)
    

where **id** (H4) and **planets** (B5:B13) are [named ranges](https://exceljet.net/glossary/named-range)
. With the number 5 in cell H2, the formula in H5 returns "Jupiter".

_Note: This example is a workaround to the problem of mismatched numbers and text, which causes VLOOKUP to return an #N/A error. If there is no mismatch, the workaround is not necessary and you can use a [normal VLOOKUP formula](https://exceljet.net/videos/how-to-use-vlookup)
._
...
Typically, the lookup column in the table contains values that _look like numbers_, but are in fact _numbers entered as text_. When a true number is passed into VLOOKUP as the _lookup\_value_, VLOOKUP returns #N/A, even though there appears to be a match. The screen below shows an example of this problem:

![Example of VLOOKUP #N/A error caused by numbers and text mismatch](https://exceljet.net/sites/default/files/styles/original_with_watermark/public/images/formulas/inline/VLOOKUP%20with%20numbers%20and%20text%20error.png?itok=4s8QbiQb "Example of VLOOKUP #N/A error caused by numbers and text mismatch")
...
All of these formulas return #N/A since they are all affected by the mismatch. One quick solution to the problem is to enter the lookup value in **id** (H4) as _text_ instead of a number. You can do this by [prefacing the number with a single quote](https://exceljet.net/videos/how-to-enter-numbers-as-text-in-excel)
 ('). VLOOKUP will then correctly find the table and perform the lookup. A better solution is to make sure the lookup values in the table are indeed numbers. Once you have converted the first column to numeric values, the standard VLOOKUP formulas above will work.

However, if you don't have control over the table, you can modify the VLOOKUP formula itself to change the numeric lookup value to text in order to match the lookup table. You can do this by [concatenating](https://exceljet.net/glossary/concatenation)
 an [empty string](https://exceljet.net/glossary/empty-string)
 ("") to the lookup value inside VLOOKUP. The formulas below show what this looks like. The first formula is a standard VLOOKUP, the second formula shows the modified version:
...
![Revised VLOOKUP formula to fix error](https://exceljet.net/sites/default/files/styles/original_with_watermark/public/images/formulas/inline/VLOOKUP%20with%20numbers%20and%20text%20revised.png?itok=W4SSFuls "Revised VLOOKUP formula to fix error")

If you aren't sure if the first column in the lookup table is text or numbers, use the [ISTEXT function](https://exceljet.net/functions/istext-function)
 or the [ISNUMBER function](https://exceljet.net/functions/isnumber-function)
 to test the values.

### Both numbers and text

If you can't be certain when you'll have numbers and when you'll have text, you can cater to both by wrapping VLOOKUP in the [IFERROR function](https://exceljet.net/functions/iferror-function)
 and using a formula that handles both cases:
