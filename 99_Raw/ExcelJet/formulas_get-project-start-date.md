...
**Source:** https://exceljet.net/formulas/get-project-start-date

---

[Skip to main content](https://exceljet.net/formulas/get-project-start-date#main-content)

[](https://exceljet.net/formulas/get-project-start-date#)

*   [Previous](https://exceljet.net/formulas/get-project-midpoint)
    
*   [Next](https://exceljet.net/formulas/get-quarter-from-date)
...
![Excel formula: Get project start date](https://exceljet.net/sites/default/files/styles/original_with_watermark/public/images/formulas/Get%20project%20start%20date.png "Excel formula: Get project start date")

Summary
-------

To calculate a project start date based on an end date and duration in days, you can use the WORKDAY function. In the example shown, the formula in E5 is:
...
In this case, we need to count _backwards_ from the end date, so we supply days as a negative number. For holidays, we provide the [named range](https://exceljet.net/glossary/named-range)
"holidays", which includes the dates in G5:G9. The dates will not be included as workdays.
...
If your schedule requires custom workdays (i.e. weekdays are not Monday-Friday) you can substitute the [WORKDAY.INTL function](https://exceljet.net/functions/workday.intl-function)
 for WORKDAY. For example, to calculate a start date for a project where workdays include Saturday, you could use:

    =WORKDAY.INTL(C5,-D5,11,holidays)
    

WORKDAY.INTL can be configured in many ways. [This page provides details](https://exceljet.net/functions/workday.intl-function)
.
